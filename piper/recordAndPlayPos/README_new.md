...
## Summary
This document provides two Python scripts (`recordPos_new` and `playPos_new`) for recording and playing back fixed positions of the Piper robotic arm. Through simple operations, users can record the arm's joint positions and gripper state, and play back these actions when needed.

## Features
- **Recording Function** (`recordPos_new.py`): Real-time recording of joint angles and gripper open/close state.
- **Playback Function** (`playPos_new.py`): Reads the recorded position file and controls the arm to reproduce the actions.
