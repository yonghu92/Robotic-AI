...
## Summary
This document provides two Python scripts (`recordTrajectory.py` and `playTrajectory.py`) for recording and playing back continuous motion trajectories of the Piper robotic arm. Unlike point-based recording, this system captures time interval information during arm movement, enabling smoother and more natural motion reproduction.

## Features
- **Continuous Trajectory Recording** (`recordTrajectory.py`): Real-time recording of joint angles, gripper state, and time intervals during arm motion
- **Precise Trajectory Playback** (`playTrajectory.py`): Reproduces arm motion according to the recorded timing relationships
